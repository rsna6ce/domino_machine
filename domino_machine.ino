...
void loop() {
    ...
    if (IrReceiver.decode()) {
        ...
        if (decoded == IR_STARTSTOP) {
            ...
            offset_run -= OFFSET_RUN_VALUE;
        } else if (decoded == IR_RIGHT && (offset_steering <= OFFSET_STEERING_MAX-OFFSET_STEERING_VALUE)) {
            offset_steering += OFFSET_STEERING_VALUE;
        } else if (decoded == IR_LEFT && (OFFSET_STEERING_VALUE-OFFSET_STEERING_MAX <= offset_steering)) {
            offset_steering -= OFFSET_STEERING_VALUE;
        }
        delay(100);
        IrReceiver.resume();
    ...
    if (millis() > next_millis) {
        if (next_state == arm_swing) {
            // arm
            servo1_a.write(50, 50, true);
            servo1_a.write(0, 100, true);
            next_state = run_start;
            next_millis = millis() + 0; //nowait
            
        } else if (next_state == run_start) {
            // run
            set_speed_percent(25+offset_steering, 25-offset_steering);
            next_state = run_stop;
            next_millis = millis() + 600 + offset_run;
            
        } else if (next_state == run_stop) {
            // stop
            set_speed_percent(0, 0);
            next_state = waiting;
            next_millis = millis() + DELAY_RUN_AFTER_ARM;
            
        } else if (next_state == waiting) {
            // waiting
            if (machine_running) {
                next_state = arm_swing;
                next_millis = millis() + 0; //nowait
            }
        }
    }
...
void setup() {
    ...
    servo1_a.write(0);
    set_speed_percent(0, 0);
    IrReceiver.begin(IR_PIN_NO, ENABLE_LED_FEEDBACK);
